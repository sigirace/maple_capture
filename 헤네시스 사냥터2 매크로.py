...
base_path = 'C:\\Users\\신강식\\Desktop\\macro'

# ---------- 캐릭터 인식작업 수행 함수----------------
def find_character_coordinates():
    print("캐릭터 인식 작업 수행중..")
    left_character_image = cv2.imread(os.path.join(base_path, "images\\left_char.png"), cv2.IMREAD_COLOR)
    right_character_image = cv2.imread(os.path.join(base_path, "images\\right_char.png"), cv2.IMREAD_COLOR)
    ...
    while True:
        ...
        left_result = cv2.matchTemplate(screenshot, left_character_image, cv2.TM_CCOEFF_NORMED)
        right_result = cv2.matchTemplate(screenshot, right_character_image, cv2.TM_CCOEFF_NORMED)
        threshold = 0.8

        left_similarity = cv2.minMaxLoc(left_result)[1] if cv2.minMaxLoc(left_result)[1] >= threshold else 0
        right_similarity = cv2.minMaxLoc(right_result)[1] if cv2.minMaxLoc(right_result)[1] >= threshold else 0
...
def move_character(character_x, target_x, character_direction_left):
    ...
    if (character_x > target_x and character_direction_left) or (character_x < target_x and not character_direction_left):
        ...
        movement_time = (distance - 16) / 117 * 0.5  #반대면일 경우 이동할 좌표에서 16을 뺍니다.
...
def change_channel():
    ...
    ch_image_location = pyautogui.locateOnScreen(os.path.join(base_path,"images\\ch.png"), confidence=0.7)
    ...
    # 6. 약 3분간 대기
    time.sleep(180)
    ch_image_location = pyautogui.locateOnScreen(os.path.join(base_path,"images\\mainch.png"), confidence=0.7)
    while True:
        if ch_image_location:
            ch_x, ch_y = pyautogui.center(ch_image_location)
...
def check_for_channel_change():
    while True:
        ...
        reduser_image = cv2.imread(os.path.join(base_path,"images\\reduser.png"), cv2.IMREAD_COLOR)
...
# Tesseract-OCR 설치 경로 설정
    pytesseract.pytesseract.tesseract_cmd = r'C:\Users\신강식\Desktop\macro\ocr\tesseract.exe'
...
while True:
    while True:
        ...
        for monster_image_path in monster_images:
            ...
            # 이미지 매칭
            result = cv2.matchTemplate(screenshot_bgr, monster, cv2.TM_CCOEFF_NORMED)
        ...
        if closest_monster is not None:
            ...
            if x_difference < 0:
                ...
                time.sleep(max(0, (abs(x_difference) - distance_to_monster) / 117 * 0.5))
                pyautogui.keyUp("left")
            else:
                pyautogui.keyDown("z")
                pyautogui.keyDown("right")
        ...
        # 같은 층에 몬스터가 더이상 없을 경우
            ...
            rope_image = cv2.imread("C:\\Dump\\rope.png", cv2.IMREAD_COLOR)
            rope_result = cv2.matchTemplate(screenshot_bgr, rope_image, cv2.TM_CCOEFF_NORMED)
            ...
            if len(rope_loc[0]) > 0:
                         ...
                         valid_ropes = [
                         rope for rope in rope_coordinates if abs(character_y - rope[1]) < 200
                         ]

                         if len(valid_ropes) > 0:
                             # 가장 가까운 로프 선택
                             closest_rope = min(         
                             valid_ropes, key=lambda rope: abs(character_x - rope[0])
                             )
                             ...
                             #로프 쪽으로 이동
                                 ...
                                 if rope_x < character_x:
                                     ...
                                     move_character(character_x, rope_x, character_direction_left=True)
                                 else:
                                     print(f"캐릭터를 로프쪽으로 이동합니다")
                                     move_character(character_x, rope_x, character_direction_left=False)
